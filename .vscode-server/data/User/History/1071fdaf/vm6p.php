...
class LeaveController extends Controller
{
    ...
    public function index()
    {
        ...
        $activation = EmployeeStatus::query()->select()->get()->sortBy('name');
        return view('leave.leave', compact('activation'));
    ...
    public function leaveTable(){
        ...
        // $list = Leave::get();
        $list = Leave::query()->select()->orderBy('type','ASC')->orderBy('expiration','ASC')->get();
        return DataTables::of($list)->make(true);
    }
    ...
    { 
            ...
            $leaves->activation= $request->activation;
            $leaves->expiration= $request->expiration;
    ...
    public function validate_updateleave(Request $request)
    {
        ...
        $currenttomonth = Leave::query()->select()
            ...
            ->first()
            ->to_month;
        $currentpriortoavail = Leave::query()->select()
        ->where('id',$request->id)
        ->first()
        ->no_of_days_prior_avail;
        if($currentcode ==$request->leavecode && $currenttype == $request->leavetype && $currentnoofdays == $request->noofdays
        && $currentfrommonth == $request->availmonthfrom && $currenttomonth == $request->availmonthto && $currentpriortoavail == $request->noofdaystoavail){
                // $email = 0;
                $result = "nochanges";
            }
            else{  
